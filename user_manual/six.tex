...
\item [lag] Lag angle in the cavity (zero is default)
\end{description}
...
\subsection{Dynamic Kicks} \label{sec:DYNK}
...
The settings can be computed on-the fly using several functions, loaded from input files, or a combination.
...
\begin{longtable}{|p{2.25cm} | p{4cm} p{9.5cm}|}
  %Note: \\* (with the star) used to inhibit line breaks in the middle of groups of the same type
...
\item [ntco] (integer) A switch to write out linear coupling
 ...
 \item {\em ntco} \/$\neq$ 0 : write--out of all linear coupled (4D)
   ...
   [rad]) are written in binary format on file \# 11.
 \end{itemize}
...
\todo[inline]{Missing variables on line 1}
...
\subsection{Dumping of beam population}

\subparagraph{Descrioption}
...
\todo[inline]{remove RIPP (or note obsolete), add DYNK and DUMP (alphabetic?)}
...
%\rule[-1.25mm]{0mm}{7.5mm}
  ...
  \hline \rule[-3.75mm]{0mm}{10mm}
  11 & & X & Binary & This file is used to dump linear \\*
  \rule[-3.7mm]{0mm}{7.5mm}
...
\todo[inline]{Update for dynk (input and output). Also update for Checkpoint/restart (92/93/95?) and collimation?}
